...
// We want to find out how much memory is allocated on startup to account for
// statically allocated data (e.g., large global arrays).
// When more than 4KB is needed, these show up in the "heap" map. As long as
// we can read that before any dynamic memory allocation happens, we are ok.
// But, even opening a file to read /proc/'pid'/maps causes some heap allocation
// and then the value read from the 'heap' mapping is way too large.
// So, we create a pipe and fork a child process (neither of which trigger
// dynamic memory allocation) and then have the child read the parent's maps
// and pass the resulting memory usage back through the pipe.

unsigned long
get_initial_bytes_used(struct mallinfo *start_mallinfo)
{
        char filename[128];
        char line[256];
	char mapsource[80];
	...
	if (child < 0) {
		...
		close(pipefd[0]);
		sprintf(filename, "/proc/%d/maps", getppid());
		f = fopen(filename, "r");
		assert(f);
		do {
			/* read line */
			if (fgets(line, sizeof(line), f) != NULL) {
				matches = sscanf(line,
						 ...
						 &start, &end, &ex, mapsource);
			} else {
				matches = EOF;
			}
			if ((ex != 'x' &&  (strstr(mapsource,"/sim") != NULL))
			    || (strstr(mapsource,"heap") != NULL) ) {
				unsigned long bytes = end-start;
				all_bytes += bytes;
			}
		} while (matches > 0);
		
		write(pipefd[1],&all_bytes,sizeof(unsigned long));
		close(pipefd[1]);
		exit(0);
	} else {
		// parent
		close(pipefd[1]);
		read(pipefd[0],&all_bytes,sizeof(unsigned long));
		close(pipefd[0]);
...
// This is used at the end. Don't need to check maps anymore. 
unsigned long
get_final_bytes_used(struct mallinfo *start_mallinfo)
{
	struct mallinfo end_mallinfo = mallinfo();
	unsigned long start_malbytes = start_mallinfo->hblkhd + start_mallinfo->uordblks;
	unsigned long end_malbytes = end_mallinfo.hblkhd + end_mallinfo.uordblks;
	return (end_malbytes - start_malbytes);
}
...
int main(int argc, char *argv[])
{
	...
	double starttime;
	double endtime;
	struct mallinfo start_mallinfo = {0};
	unsigned long bytes_used;       
	const char *usage = "USAGE: sim -f tracefile -m memorysize -s swapsize -a algorithm\n";
	int opt;

	// Get initial memory use (global variables)
	bytes_used = get_initial_bytes_used(&start_mallinfo);
	...
	//(void)replay_trace;
	endtime = get_time();
	bytes_used += get_final_bytes_used(&start_mallinfo);
