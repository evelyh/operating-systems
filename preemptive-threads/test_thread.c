...
test_wait(void)
{
	...
	__sync_fetch_and_add(&done, 1);
	for (i = 0; i < NTHREADS; i++) {
		thread_wait(wait[i], &exitcode);
		assert(exitcode == (i + THREAD_MAX_THREADS));
	}
...
test_wait_parent_thread(Tid parent)
{
	...
	/* 1 in 10 threads stalls a long time, so parent should exit first */
	if (thread_id() % 10 == 0) {
		spin(SIG_INTERVAL * 50);
	}

	Tid ret = thread_wait(parent, &exitcode);

	// check exitcode - parent exits with -42
	// At least 1 child must get exitcode. Others may get THREAD_INVALID
...
/*
 * Parent thread creates N children threads. Children threads then wait on the
 * parent thread. Then parent kills roughly N/2 children threads via
 * thread_kill. Parent thread then calls thread_exit. The killed children 
 * threads should not run any further, while the rest of the children should 
 * run to completion. For each child thread, we should see one of 
 * "thread woken", "parent gone", or "thread killed".
 * 
 * NOTE: If a killed thread has not been waited on yet, it should be 'detached'
 * indicating that it will never be waited on. 
 */
void
